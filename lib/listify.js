  ...
  let buildList = function(config) {
    ...
    let weirdStrategy = function() {
      ...
      let parts = bodyparts(), //corpora.getFile('humans', 'bodyParts').bodyParts,
          ...
          list = new Array(50).fill().map(() => template());
      return {
        filtered: list.sort(),
    ...
    if (config.method) {
      let FuzzyMatching = require(`fuzzy-matching`),
          buildMethods = [`matchStrats`, `posStrats`, `posStratAdjs`, `weirdStrats`, `patternStrats`],
          fmMethods = new FuzzyMatching(buildMethods),
          method = fmMethods.get(config.method).value;
    ...
    var list = (config.matchPattern
                ...
                : util.pick(strategy ? strategy : strategies))();
