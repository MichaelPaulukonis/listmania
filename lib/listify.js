...
let Listmania = function() {
  ...
  let textFilter = function(text, matchPattern) {
    ...
    let strategies = [
      ...
      // `. #Value .`
      // `. (#Date|#Value) .`
      // Adjective, PastTense, Gerund, Superlative, Comparative, Determiner, Demonym, Date,
      // Value, Adverb, City, Organization, Copula, Person, Noun, FemaleName, MaleName
      // LastName, Quotation, Expression, Country, Month, WeekDay, Duration, Holiday
      // Possessive, Preposition, Singular, Plural, Currency, Unit,  Infinitive, Contraction
      // Pronou, Ordinal, Cardinal, TextValue, NumericValue, Auxillary, Negative, PresentTense
      // FutureTense, Participle, Conjunction, QuestionWord, Honorific
      // ['PastTense', 'PerfectTense', 'Pluperfect', 'FuturePerfect', 'Copula', 'Modal', 'Participle', 'Infinitive', 'Gerund']
      // #NounPhrase+, #VerbPhrase+ - don't really work without the "+"
  ...
  let getList = function(text, matchPattern) {
    ...
    let strategy = textFilter(text, matchPattern),
        pos = strategy.parts,
        subset = pos;
